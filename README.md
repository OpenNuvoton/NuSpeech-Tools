...
[Features]
1. New playback features for NSP2
   - Avoid playback noise at playback volume is smaller
   - New commands to playback encoded audio sent by host MCU
   - New commands to playback configurable frequency of sine or square waveform (1Hz~16000Hz)
2. Support to add encoded audio data (.rbf) as resource item and build project on NSP1 chip project
   Extract .rbf files in .obj file from menu [Project|Extract Encoded data from OBJ file]
3. Embed an AI chat window for answering questions about NSP GUI and host driver samples
4. Add "APP" folder containing encode and decode demo sample code for the Android, iOS, and Linux platforms
5. Update host samples with these new features
   a. playback encoded audio sent by host MCU
   b. playback sine or square waveform
6. Update EQ using normalization method to prevent PCM clipping
7. Renew documents
   a. AI chat window instructions in user guide document
   b. New NSP3 chip MCU commands document
   c. New commands to playback encoded audio and configurable frequency of sine or square waveform
8. Support for volume boost, with a maximum boost of 100 dB.

[Bug-Fixed]
1. Fixed an issue where EQ configuration files were not saved correctly in multilingual environments
2. Fixed an issue where duplicate project data was not copied to the EQ configuration file
3. Fixed an issue where network roaming caused frequent TTS connection drops
...
5. pop the floating prompt window to show different remain time of different sample rate8K,12K,16K. or rate8K,10K,12K. setting when mouse move on the remain block of status bar
...
[Bug-Fixed]
1. NSP040 resource window hide unsupported menu items [Volume Boostdb.] and [Change Bit Rate]
...
---------------------------------------------------
[Features]
1. Support P16 resource format on NSP1exclude NSP040, NSP OTP. and NSP2 project
...
[Note]
If the NSP2340 project support all resource update function on SP2, need to rebuild project and write project on chip after updating to SP3
...
2. support NSP1 series chip: NSP171A53A/NSP341A53A
3. support to change output file name on NSP chip project*.obj and *.rtf.
4. support to switch chip between NSP040 chip and other NSP chipNSP080, NSP340, etc.... on project setting page
...
2. demo mode support mixed pinsBP00+BP01. on SOP8 packet
...
6. New function is “Save ProjectInclude Resource Files.” to copy resource files to local project folder.
...
[Fix]
1. incorrect sort after inserting multiple Audios right on Index.
2. delete the resource in sound editing used by the demo mode switching demo mode will cause crash.
3. setting operation, press Enter on the keyboard, some icons will disappear.
4. demo Mode adds resource in the blank space, additional branches will appear.
5. The sampling rate of the Chinese wav file exceeds 16000, the post-build message is not Chinese.
6. One Wire does not support User Data Size suggestion field is grayed out.
...
2. provide reserved user space function on build setting
   user can get the user space informationISP address, user space size. and access using ISP read and write partial command
...
2. add support NSP OTP chip include NSP075/165/335.
...
6. add when resample out of range need to popup warning message.
7. add export full error log file to verify problem.

[Fix bugs]
1. fixed device not found, but when build and write finish, the message is write ok#2919.
2. fixed parameter error and crash while type volume boost.
3. fixed when demo mode index have resource but repeat is 0 when open befoere V1.09.000 version.
4. fixed can not set 0 to disable reSample.
5. fixed default should focus on "OK" while New Project Settings Window.
6. fixed always highlight after cancel drag resource to add audio.
7. fixed whole index should not be selected after delete the final audio.
8. fixed wrong dispaly if close tool at max size then open again.
9. fixed select resouce that reaction to slow.
...
[Features]
 ...
 2. Add repeat command & stop repeat function.
 3. Remove unnecessary information and file of playlist tool.
 4. support setting sound quality level in resource window.
 5. Move "PLAY" command to be the first command in Sound & IO Preview window.
 6. Support POR to play sound in demo mode.
 7. Add BP06 and BP13 normal pin at SOP14 when at demo mode.
 8. Support new HOST ISP command with checksum verification.

[Fix bugs]
 1. execute two Playlist Editor Tool at the same time lead to write function fail.	
 2. execute Playlist Editor Tool and ICP Writer at the same time lead to write function fail.
 3. popup update F/W when execute Sound & IO Preview function sometimes.
 4. the resource path is not found, build+write will still succeed.
 5. Sound & IO Preview function should not be executed when in demo mode.
 6. Add resource that estimate percentage inaccuracy.

---------------------------------------------------
NSP PlayList Editor Tool V1.08.000
---------------------------------------------------
[Features]
 1. NSP PlayListEditor Tool uninstall can not clear all file.
 2. Use ISP online update audio resource.
 3. Build will no action after open project.
 4. GUI no action at loading a large of resource files into resource window		
 5. When create new project can immediately to set option.	
 6. Support chinese folder name.	
 7. Select/Delete: take too much time at add or del resource file in resource window
 8. Resource Editor added wave file can not be save.
 9. Using the hotkey Delete to delete more than 5 consecutive times will cause the NSP PlayList Editor to stop
 10. Sound and IO Preview Index over 50 PLAY will be Invalid.
 11. Build project demo or cpu mode be copy object file from Code_Release to project folder, and only one mode object file exist.
 12. Change sample rate but this project will not build if this project was built before.	
