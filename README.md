...
==================2021/02/25======================
(1) cancel the power amplifier function on project setting
(2) support POR function on sound editing mode for NSP and OTP chip
(3) fix the update resource cancel function doesn't work problem on update resource window
(4) support to response the tool version on two wire mode or UART mode
(5) Sound Editing mode will check the available index number to avoid build project fail
	When edit index number on Sound Editing page, it will check the index is out of range or not
	When build project on Sound Editing page, it will check all index are out of range or not

================================================== 
======NSP Playlist Editor Tool V1.12.036    	 =
==================2021/01/13======================
(1) support new method to estimate resource and playlist table size precisely on NSP chip project
(2) OBJ file's header should contain tool version not packer version
(3) change request form file format
	a. Change title ""Nuvoton PowerSpeech?: NuSpeech Request Form"" to ""Nuvoton NSP PlayList Request Form""
	b. Revise ""S/W"" from SDS version to NSP playlist tool version"
(4) add the function to force encode all resources and estimate size on menu
(5) the application will encode all resources and estimate size when the silence compression enable setting is changed

================================================== 
======NSP Playlist Editor Tool V1.12.035    	 =
==================2021/01/05======================
(1) fix the multiple play will skip to play some file on NSP OTP chip
(2) update PlaylistEditorGen.dll to sort the chip name using chip number on project setting
(3) support to show remain time (second) on bottom status bar
(4) record the sound & I/O preview current height and width information when window close, when window open will refer this information and resize window
(5) fix the resource window layout moving problem if install service pack and open exist project
(6) fix crash problem after switch the the display format from decimal to hexadecimal on sound & I/O preview

================================================== 
======NSP Playlist Editor Tool V1.12.034    	 =
==================2020/12/21======================
1. Update firmware assembler code to support these feature or fix bugs
  (1) fix the problem that maybe lost repeat command on one wire or two wire mode
  (2) one wire multi-play command index number from 10 to 32 for NSP chip(all) and NSP OTP chip(NSP335)

2. Update NSP ICP Tool V1.07.107 to support new ISP one wire multiple play command
3. Update mass storage firmware (54.0C.10.15) to support new ISP one wire multiple play command
4. Update document to application directory
  (1) NSP_OneWire MCU Interface_CHS_A1.3.pdf
  (2) NSP_OneWire MCU Interface_EN_A1.3.pdf
  (3) NSPOTP_OneWire MCU Interface_CHS_A1.2.pdf
  (4) NSPOTP_OneWire MCU Interface_EN_A1.2.pdf
  (5) NSP_TwoWire MCU Interface_EN_A2.7.pdf
  (6) NSP_TwoWire MCU Interface_CHS_A2.7.pdf
  (7) NSP PlayList Editor Tool User Guilde.pdf

================================================== 
======NSP Playlist Editor Tool V1.12.033    	 =
==================2020/12/11======================
1. Update firmware assembler code to fix these bugs
	(1) NSP & OTP one-wire multi-play error
	(2) NSP two-wire ISP update slowly
	(3) NSP two-wire ISP update single resource and power off may play resource fail
2. Update mass storage firmware (54.0C.08.0C) to act in concert with new assembler code
3. Update document to application directory
	(1) NSP_TwoWire MCU Interface_EN_A2.6.pdf
	(2) NSP_TwoWire MCU Interface_CHS_A2.6.pdf
4. Update NSP ICP Tool V1.07.106 to cancel to check chip is free when run these ISP commands
	(1)ISP_WRITE_START
	(2)ISP_WRITE_PARTIAL_ START
	(3)ISP_WRITE_END
	(4)ISP_WRITE_PAGE
	(5)ISP_WRITE_PARTIAL_BAK
	(6)ISP_WRITE_PARTIAL
	(7)ISP_READ_RES_INDEX
Note:
Because the HID protocol firmware not update, so the update all playlist table and update single resource will fail using HID dongle

================================================== 
======NSP Playlist Editor Tool V1.12.032    	 =
==================2020/11/18======================
1. NSP OTP project support silence condition function
2. NSP project support set PCM and limit value for silence condition function
3. Support NSP082/NSP172 chip

================================================== 
======NSP Playlist Editor Tool V1.12.031    	 =
==================2020/09/21======================
[Features]
1. when build project successfully, create the directory name "??厂" on project folder and the object on this folder for burning factory
